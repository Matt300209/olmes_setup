...
class VLLM_Verbose(VLLM):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        print(f"Class is defined in: {file_path}")
        print("___________________________________________________")
        
        # add prometheus logger
    ...
    def get_metrics(self):
        """Get metrics from vLLM 0.8.5 engine"""
        if not hasattr(self, 'model') or not hasattr(self.model, 'llm_engine'):
            print("check no llm_engine")
            return {}
    ...
    # oe-eval: Convert compute context and all_gen_kwargs in a custom way, then minimal changes needed below
        ...
        for chunk in chunks:
            ...
            metr = self.get_metrics()
            elapsed_time = end - start
            throughput_dict["total_elapsed_time"] = elapsed_time
            throughput_dict["num_req"] = int(self.batch_size)
            num_ctx_tokens = 0
            num_cont_tokens = 0
            # cache generations
            for output, context, context_length in zip(cont, context, context_lengths):
                ...
                pbar.update(1)
                num_ctx_tokens += context_length
                num_cont_tokens += num_tokens
            ...
            throughput_dict["req/s"] = int(self.batch_size) / elapsed_time
    ...
    def _loglikelihood_tokens_verbose(
        ...
        for chunk in chunks:
            ...
            for output, ctxlen, (cache_key, _, _), inp in zip(outputs, ctxlens, chunk, inputs):
                ...
                num_tok = len(completion_output.token_ids)
                num_cont_tokens += num_tok
                res.append(answer)
